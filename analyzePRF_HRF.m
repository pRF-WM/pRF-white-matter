function results = analyzePRF_HRF(stimulus,data,tr,options,hrfpara)
...
%     NOTE: setting this input properly might be critical to avoid local minima.
%     The <typicalgain> value is used as an initial seed, and we have seen instances
%     where if the <typicalgain> is much higher than the optimal value, then the
%     optimizer has a hard time finding a good solution.  So, if you have converted
%     your data to units of percent BOLD change, you should probably set <typicalgain>
%     to a small value like 0.1.  The default is 10, which is probably reasonable
%     if your data are in raw scanner units.
%
...
fHRF4=@(nPara,nTime) vflatten((nTime.^(nPara(1)-1).*nPara(2).^nPara(1).*exp(-nPara(2).*nTime))./gamma(nPara(1))); %  gamma hrf from SPM
% load('E:\WH\analyzePRF\avg178\Vari_pRF_HRF_gridseed.mat')
hrfmethod = 2; % 2 for double gamma, 5 for single gamma
...
if hrfmethod ==2
    modelfun = @(pp,dd) conv2run(posrect(pp(4)) * ... gain
        (dd*[vflatten(placematrix(zeros(res),...
        makegaussian2d(resmx,pp(1),pp(2),abs(pp(3)),abs(pp(3)),xx,yy,0,0) /...
        (2*pi*abs(pp(3))^2))); 0]) .^ ...
        posrect(pp(5)),...
        fHRF2( pp(6:10), nTimeHRF ),...
        dd(:,prod(res)+1));
    %SPM's two gamma hrf function
    %pp(6)  a1, time to response, seconds (default: 6)
    %pp(7)  b1, response dispersion (default: 1)
    %pp(8)  a2, time to undershoot, seconds (default: 16)
    %pp(9)  b2, undershoot dispersion (default: 1)
    %pp(10)  c, response undershoot ratio (default: 6)
    %pp(11)  delta, onset, seconds (default: 0)
    model = {{[] [1-res(1)+1 1-res(2)+1 0    0   NaN  0   0   0   0   0;
                  2*res(1)-1 2*res(2)-1 Inf  Inf Inf  Inf  Inf  Inf  Inf  Inf] modelfun}}; % for spm hrf
    % init
    seeds = [];
    ...
    if ismember(0,options.seedmode)
        seeds = [seeds;
            (1+res(1))/2 (1+res(2))/2 resmx/4*sqrt(0.5) options.typicalgain 0.5 6 1 16 1 6]; % for spm hrf
    end
    ...
    if any(ismember([2 -2],options.seedmode))
        [supergridseeds,rvalues] = analyzePRFcomputesupergridseeds(res,stimulus,data,modelfun, ...
            ...
            options.typicalgain,noisereg,hrfpara);
    end
elseif hrfmethod ==5
    modelfun = @(pp,dd) conv2run(posrect(pp(4)) * ... gain
        (dd*[vflatten(placematrix(zeros(res),...
        ...
        (2*pi*abs(pp(3))^2))); 0]) .^ ...
        posrect(pp(5)),...
        fHRF4( pp(6:7), nTimeHRF ),...
        dd(:,prod(res)+1));
    model = {{[] [1-res(1)+1 1-res(2)+1 0    0   NaN 0 0 ;
        2*res(1)-1 2*res(2)-1 Inf  Inf Inf 10 10 ] modelfun}};
    seeds = [];
    if ismember(0,options.seedmode)
        seeds = [seeds;
            (1+res(1))/2 (1+res(2))/2 resmx/4*sqrt(0.5) options.typicalgain 0.5 6 1];
    end
    if ismember(1,options.seedmode)
        seeds = [seeds;
            (1+res(1))/2 (1+res(2))/2 resmx/4*sqrt(0.5) options.typicalgain 0.5 6 1];
...
if wantquick
  ...
  opt = struct( ...
    ...
    'model',{model}, ...
    'seed',seedfun, ...
    'optimoptions',{{'Display' options.display 'Algorithm' 'levenberg-marquardt'...
...
if hrfmethod == 2
    % for spm hrf
    results.a1(options.vxs,:) = permute(paramsA(:,6,:),[3 1 2]);
    results.b1(options.vxs,:) = permute(paramsA(:,7,:),[3 1 2]);
    results.a2(options.vxs,:) = permute(paramsA(:,8,:),[3 1 2]);
    results.b2(options.vxs,:) = permute(paramsA(:,9,:),[3 1 2]);
    results.c(options.vxs,:) = permute(paramsA(:,10,:),[3 1 2]);
elseif hrfmethod == 5
    results.a1(options.vxs,:) = permute(paramsA(:,6,:),[3 1 2]);
    results.b1(options.vxs,:) = permute(paramsA(:,7,:),[3 1 2]);
end
...
if hrfmethod == 2
    % for spm hrf
    results.a1       = reshape(results.a1, [xyzsize numfits]);
    results.b1       = reshape(results.b1, [xyzsize numfits]);
    results.a2       = reshape(results.a2, [xyzsize numfits]);
    results.b2       = reshape(results.b2, [xyzsize numfits]);
    results.c        = reshape(results.c, [xyzsize numfits]);
elseif hrfmethod == 5
    results.a1       = reshape(results.a1, [xyzsize numfits]);
    results.b1       = reshape(results.b1, [xyzsize numfits]);
end
