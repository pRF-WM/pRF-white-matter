...
%% for multistart
%         lb = [0 0 0 0 0];
%         problem = createOptimProblem('lsqcurvefit','objective',@(x,y) double(nanreplace(feval(fun,x) / datastd,0,2)),...
%             'x0',seed(ix),'xdata',[],'ydata',double(traindataT),'lb',lb,'ub',ub,'options',options);
%         ms = MultiStart('Display','iter');
%         [params0,resnorm,exitflag,outptm,manyminsm] = run( ms, problem, 9 );
%         output=manyminsm(1).Output;
%% end
        
        % wang xiaoxiao's code
%         try
%           oOptimProbl = createOptimProblem( 'lsqcurvefit', 'x0', seed(ix), 'lb', lb, ...
%                 'ub', ub, 'ydata', double(traindataT), 'options', options, ...
%                 'objective', @(x,y) double(feval(fun,x) / datastd ));
%           oMS = MultiStart;
%           [params0,resnorm,exitflag,~,manyminsm] = run( oMS, oOptimProbl, 9 );
%           output=manyminsm(1).Output;
%           seed=manyminsm(1).X0{1};
%           
