function [seeds,rvalues] = analyzePRFcomputesupergridseeds(res,stimulus,data,modelfun,maxpolydeg,dimdata,dimtime,typicalgain,noisereg,hrfpara)
...
% MODIFICATION FOR HCP 7T RETINOTOPY DATASET.
% Here, we use a denser grid for eccentricity and angle.
% Also, we fix the exponent to 0.05.
eccs = [0 0.00551 0.014 0.0269 0.0459 0.0731 0.112 0.166 0.242 0.348 0.498 0.707 1];
eccs = [eccs (eccs(1:end-1)+eccs(2:end))/2];
angs = linspacecircular(0,2*pi,32);
expts = .05;
...
% MODIFICATION FOR HCP 7T RETINOTOPY DATASET.
% Here, we use a denser grid for sigma.
ssindices = 2.^(0:.5:maxn);


% construct full list of seeds (seeds x params) [R C S G N]
fprintf('constructing seeds.\n');
allseeds = zeros(length(eccs)*length(angs)*length(ssindices)*length(expts)*(5^5),10);
cnt = 1;
for p=1:length(eccs)
  for q=1:length(angs)
...
parfor p=1:size(allseeds,1) % parfor p=1:size(allseeds,1)
...
parfor p=1:length(chunks) % parfor p=1:length(chunks)
